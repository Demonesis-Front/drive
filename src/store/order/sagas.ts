...
import { PositionStack } from 'services/positionstack/service';
  ...
  try{
    ...
    // Cities
      ...
      for(let i = 0; i < cities.length; i++){
        const points: LatLngExpression = yield call(PositionStack.getCity, cities[i].name)
        delay(2000)
    ...
    // Pickups
      ...
      for(let i = 0; i < points.length; i++){
        if(points[i] && points[i].name && points[i].cityId && points[i].cityId.name){
          const coordinates: LatLngExpression = yield call(PositionStack.getPoints, points[i].address, points[i].cityId.name)
